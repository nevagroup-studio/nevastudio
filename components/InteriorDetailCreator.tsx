...
import { generateInteriorDetailVariationSet } from '../services/geminiService';
import { translations } from '../locales/translations';

interface InteriorDetailCreatorProps {
    onBack: () => void;
...
export const InteriorDetailCreator: React.FC<InteriorDetailCreatorProps> = ({ onBack, addImageToLibrary, addHistoryItem, aiModel, setFullscreenData }) => {
    ...
    const viewTypes = translations[language].interiorDetailViewTypes;
    const optionNames = [
        viewTypes.detail1, viewTypes.detail2, viewTypes.detail3,
        viewTypes.detail4, viewTypes.detail5, viewTypes.detail6,
        viewTypes.artistic1, viewTypes.artistic2, viewTypes.artistic3
    ];
    ...
    const handleGenerate = async () => {
        ...
        try {
            await generateInteriorDetailVariationSet(sourceImage, language, aspectRatio, imageSize, aiModel, (optionName, imageUrl) => {
            ...
            if (finalImages.length > 0) {
                 await addHistoryItem({
                    tab: 'interiorDetailView',
                    sourceImage,
                    referenceImage: null,
                    prompt: `[Interior Detail Views] Generated 9 close-up options from reference with ratio ${aspectRatio} and size ${imageSize}`,
            ...
            console.error("Interior Detail View Creation failed:", error);
    ...
    const downloadAll = () => {
        results.forEach((r, i) => {
            if (r.url) {
                ...
                link.href = r.url;
                link.download = `interior-detail-${r.name.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}.png`;
                link.click();
            }
        });
    };
            ...
            <div className="flex items-center justify-between mb-8 border-b border-slate-700 pb-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className={`text-2xl font-bold ${theme.textMain}`}>{t('interiorDetailTitle')}</h2>
                            <p className={`text-sm ${theme.textSub}`}>{t('interiorDetailDesc')}</p>
                        </div>
                    </div>
                </div>
                                ...
                                {opt.url ? (
                                    <>
                                        ...
                                        <div className="absolute top-2 right-2 z-10">
                                            <a 
                                                href={opt.url} 
                                                download={`interior-detail-${opt.name.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}.png`} 
                                    ...
                                    <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                                        ...
                                        <p className="text-[10px] text-slate-400 animate-pulse">{opt.name}</p>
                                    </div>
                                ) : (
                                    <div className="w-full h-full flex flex-col items-center justify-center opacity-30">
                                        <Icon name="camera" className="w-10 h-10 text-slate-600 mb-2" />
                                        <p className="text-[10px] uppercase tracking-widest font-bold text-slate-500">{opt.name}</p>
                                    </div>
                                )}
                            </div>
                        ))}
